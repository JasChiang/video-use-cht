...
> **ElevenLabs Scribe** 轉錄後端換成**本機 [Breeze-ASR](https://huggingface.co/MediaTek-Research/Breeze-ASR-25)（透過 WhisperX）**。
...
- **自動調色**每一段（電影暖調、中性，或任何自訂 ffmpeg 串接）
...
|---|---|---|
| 轉錄 | ElevenLabs Scribe（雲端、付費） | **Breeze-ASR-25/26 透過 WhisperX（本機、免費）** |
...
沒有單一模型同時最擅長中、英、台三者，而且不能在同一段音訊上同時跑兩個。預設用 `breeze25`，台語很重的片段才切到 `breeze26`。
...
分人用的是 gated 的 pyannote 模型。要啟用：到 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 建立一個 token，並到 [`pyannote/speaker-diarization-community-1`](https://huggingface.co/pyannote/speaker-diarization-community-1) 按下同意（pyannote.audio 4.x 旗艦 pipeline）。**沒有 token 也能轉錄**，只是全部標成 `speaker_0`。
...
- **為什麼用 WhisperX，而不是直接用 Breeze。** video-use 有條硬性規則：**只用逐字時間戳**——每個剪接點都 snap 到字邊界。但 Whisper 家族（含 Breeze）對中文的**原生逐字時間戳會抖**，因為中文沒有詞邊界、時間是用 cross-attention 推出來的。WhisperX 把 Breeze 的轉錄再經過 **wav2vec2 強制對齊**，得到穩定得多的逐字時間；而且它把 **pyannote 分人**包在同一道流程裡，連講者標籤一起搞定。所以 WhisperX = Breeze 的準度 + 可靠時間戳 + 分人，一次到位。
...
- **為什麼用 pyannote `speaker-diarization-community-1`。** 一開始鎖定 `3.1`，但 pyannote.audio 4.x 即使你要 3.1 也會去抓 `community-1` 的檔案，所以直接 pin `community-1`（你要按下同意的就是它）。

- **為什麼在 Apple Silicon 跑 CPU。** CTranslate2（faster-whisper 的引擎）沒有 Metal/MPS，所以 Breeze 跑 CPU。對**離線**剪輯來說，這是換取「本機 + 免費」的合理代價；對齊與分人仍可用 MPS。

- **CJK 打包修正。** WhisperX 對中文逐字對齊（連英文也拆成單一字母），導致打包後的逐字稿讀起來像 `大 家 好` / `v i d e o`。`pack_transcripts.py` 現在改成 CJK 感知的接合：任何 CJK 邊界或兩個單一 ASCII 字之間都不空格，只有真正的多字母英文詞之間才空格。

這是個反覆迭代的過程——包含只有實際端到端跑才會抓到的真 bug（被改名的 `token` 參數、`community-1` 門禁、無害的 `torchcodec` 警告）。[疑難排解](#疑難排解)表就是這些經驗的精華。

## 抓出 Whisper 藏起來的結巴(本地 verbatim CTC 軌)

Breeze(跟所有 Whisper 家族一樣)訓練在乾淨字幕上,會**把結巴正規化掉**:口吃的 `去去去` 會被壓成一個 `去`,旁邊的時間戳被拉長來填音訊。重複在音訊裡、卻不在逐字稿裡 —— 所以「靠逐字稿剪」看不到它們。這就是「把卡頓講話剪順」對 Whisper 這麼難的根本原因。

`helpers/detect_disfluencies.py` 用**兩條本地軌**繞過去:

- **Breeze** = 準確但**正規化**的逐字稿(一個 `去`)
- **wav2vec2 CTC**(WhisperX 已快取的中文模型)= **verbatim 軌** —— CTC 沒有壓重複的語言模型,會原樣吐出 `去去去` 加上逐字時間

兩軌交叉比對:

```
CTC 有重複 + Breeze 壓掉了   →  真結巴  →  砍掉多餘的
CTC 有重複 + Breeze 也有      →  真詞    →  保留(媽媽 / 等等 / 看看 / 走走)
```

```bash
python helpers/detect_disfluencies.py clip.mp4      # 寫出 edit/disfluencies_<stem>.json
```
...
| `GatedRepoError: ... speaker-diarization-community-1 ... not in the authorized list` | 用**跟 `HF_TOKEN` 同一個帳號**到[模型頁](https://huggingface.co/pyannote/speaker-diarization-community-1)按下同意，再重跑。pyannote.audio 4.x 即使你指定舊的 `3.1` 也會用 `community-1`。 |
| `torchcodec ... Could not load libtorchcodec` 警告 | **無害**。whisperx 用 in-memory 音訊餵給 pyannote，根本不會呼叫 torchcodec（警告是因為 Homebrew 的 ffmpeg 是 8，而 torchcodec 要 4–7）。 |
| 轉錄很慢 | 正常。CTranslate2 在 Apple Silicon 沒有 Metal/MPS，Breeze 跑 CPU（約 1.2× 即時），離線剪輯沒問題。第一次還會下載約 3–4 GB 權重（一次性）。 |
...
**第二層——視覺合成（按需）。** `timeline_view` 對任意時間範圍產出膠捲縮圖 + 波形 + 字標 PNG，只在決策點呼叫。
...
**相對於上游的更動：** 把 ElevenLabs Scribe 後端（`helpers/transcribe.py`）換成本機 Breeze-ASR/WhisperX 後端（`helpers/transcribe_breeze.py`）；相應更新 `pyproject.toml`、`.env.example` 與本 README。其餘剪輯邏輯皆為上游所有。
