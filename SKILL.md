...
6. **Never cut inside a word.** Snap every cut edge to a word boundary from the Breeze/WhisperX transcript.
7. **Pad every cut edge.** Working window: 30–200ms. wav2vec2-aligned timestamps drift 50–100ms — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached transcript JSON (Breeze/WhisperX, Scribe-shaped words[])
...
- Transcription is **local** (Breeze-ASR via WhisperX) — no transcription API key needed. `HF_TOKEN` is optional and only enables pyannote diarization; if it's missing, transcription still runs and every word is tagged `speaker_0`. For multi-speaker footage, ask the user to paste a token into `.env` at the repo root (never the user's `<videos_dir>`) and accept the gated pyannote terms.
...
- **`transcribe.py <video>`** — single-file local Breeze-ASR (WhisperX). `--num-speakers N`, `--model breeze25|taigi`, `--no-diarize` optional. Cached.
- **`transcribe_batch.py <videos_dir>`** — batch transcription (local CPU, serialized). Use for multi-take. Same `--model` / `--no-diarize` flags.
...
- **Whisper SRT / phrase-level output.** Loses sub-second gap data. Always word-level (WhisperX alignment).
- **Expecting verbatim fillers from Breeze.** Whisper-family ASR (Breeze included) tends to DROP `um`/`uh`/false starts, so transcript-driven filler removal is weaker than Scribe's verbatim mode. Silence-gap cutting is unaffected (timestamps stay exact) — lean on silence gaps and `timeline_view` review for filler/dead-air, and tell the user filler-word cutting is best-effort here.
